"""Run LLM evaluation over curated haiku scenarios."""
...
def build_prompt(scenario: Scenario) -> str:
    ...
    return template.format(subject=scenario.subject)
    ...
    with predictions_path.open("a" if completed else "w", encoding="utf-8") as out:
        for model_name in models:
            ...
            for scenario in subset:
                ...
                out.flush()
                print(f"[{model_name}] {scenario.id} -> {pred.line1[:40]}...")
                time.sleep(0.2)
...
def main(argv: list[str] | None = None) -> int:
    load_dotenv()
    parser = argparse.ArgumentParser(description="Run haiku eval against LLMs")
